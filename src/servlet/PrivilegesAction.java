...
public class PrivilegesAction {
     ...
     */
    public String doListPrivileges(HttpServletRequest request, HttpServletResponse response)
            throws UnsupportedEncodingException, RoleException {
            request.setCharacterEncoding("UTF-8");
            //设置下拉框的值
            ...
            if ( !("请选择".equals(roleName) || "".equals(roleName))){
                columnName[0] = "role_name";
            }
            if ( !("请选择".equals(menuName) || "".equals(menuName))){
                columnName[1] = "menu_name";
            }
        ...
        try {
            ...
            int sum = userPrivileges.listPrivilegesByConditionOrAll(columnName,false,
                                                                    roleName, menuName).size();
            Map<String,Integer> map = divisionPage(COUNT,page,sum);
            int allPages = map.get("allPages");
            page = map.get("page");
            List<UserPrivileges> listPrivileges = userPrivileges.listPrivilegesByConditionOrAll(columnName, true,
                    roleName, menuName, (page - 1) * COUNT);
            request.setAttribute("page",page);
            //尾页
            request.setAttribute("allPage",allPages);
            request.setAttribute("listPrivileges",listPrivileges);
